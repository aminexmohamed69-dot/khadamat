    ...
    <div className="w-full h-full flex flex-col gap-8 animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="space-y-2 text-center md:text-right">
          ...
          <p className="text-blue-100/60 font-medium font-Cairo">
            {type === 'land' ? `مشاهدة المخططات (${currentIndex + 1} / ${images.length})` : 'المخطط الهندسي الكامل للمشروع'}
          </p>
        </div>
      ...
      {/* LAND PLOTS VIEW: Image Carousel Only */}
      {type === 'land' && (
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-4 mt-4">
            <h4 className="text-xl font-bold text-white drop-shadow-md">معرض الصور</h4>
            <p className="text-blue-100/70">تصفح صور المخططات بتفصيل أكبر</p>
          </div>

          {/* Single Image Carousel */}
          <div className="relative bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-2xl group ring-1 ring-black/5 h-[500px]">
            <div className="absolute inset-0 flex items-center justify-center p-4 cursor-pointer" onClick={() => setSelectedImage(images[currentIndex])}>
              <img
                src={images[currentIndex]}
                alt={`Plan ${currentIndex + 1}`}
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2">
              {images.map((_, idx) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${idx === currentIndex ? 'w-8 bg-blue-500' : 'w-2 bg-gray-300 hover:bg-gray-400'
                    }`}
                />
              ))}
            </div>
      ...
      {/* Full Screen Image Modal (Only accessible from land UI) */}
      {selectedImage && (
        <div 
